...
class main_hub(object):
    def __init__(self):
        ...
        sub = rospy.Subscriber('/varijabla_funkciji', poruka, self.callback, queue_size=1)

        self.pubs = {}
        for varijabla in self.received:
            self.pubs[varijabla] = rospy.Publisher('/{}/funkcija_varijabli'.format(varijabla), poruka, queue_size=1, latch=True)
